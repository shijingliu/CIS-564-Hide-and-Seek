...
extern "C"
{
	...
	static Flee* FLEEList; 
	static Wander* WanderList; 
	...
	void EXPORT_API initialize(int agentcount)
	{
		...
		FLEEList = new Flee[agentcount]; 
		WanderList = new Wander[agentcount];
	}
	...
	void CalculateDesiredVelocity(int agentID, vec3 target, vec3& dvel)
	{
		...
		{
			FLEEList[agentID].setTarget(target);
			dv = FLEEList[agentID].CalculateDesiredVelocity(A); 
		}

		else if (behavior == wander)
		{
			dv = WanderList[agentID].CalculateDesiredVelocity(A);    
		}
	...
	void EXPORT_API setFlee(int agentID, float* target)  
	{
		//FLEEList[agentID] = vec3 (target[0], target[1], target[2]); 
	}
